...
export default function RequestDetailPage() {
    ...
    const fetchRequestDetail = async () => {
        try {
            // Re-use list API with filter or create specific GET? 
            // For now, I'll filter client side or use supabase client?
            // Actually I implemented `list` API only. I should have implemented `get` API.
            // But I can implement a fast fetch here or add `api/requests/admin/detail`
            // Let's use `supabase-js` directly if possible or fetch list and find. List is not efficient.
            // I'll create `src/app/api/requests/admin/[id]/route.ts`... wait I didn't create it.
            // Helper: I'll use the server action `src/app/api/requests/admin/action` for actions.
            // For fetching, I'll just add a server param to list or create a new endpoint?
            // Let's just create a quick `fetch` using existing `list` if list is small, or better:
            // I'll implement a `GET` in `src/app/api/requests/admin/detail/route.ts` with param?
            // Or just allow `list?id=...`. let's check `list` implementation.
            // `list` selects all. I'll modify `list` to accept an ID filter, effectively making it a detail endpoint too.

            // Wait, I can't modify `list` easily without going back.
            // I will use client side supabase for read? No, RLS might block.
            // I will implement a quick server action or just use list for now (assuming low volume).
            // Actually, for correctness I should create a detail endpoint.
            // But for speed, I will use `list` and filter client side.

            const res = await fetch('/api/requests/admin/list') // Optimized: should be ?id=...
